...
#===============================================================================
# Szablon projektu: Generator SVG i Konwerter JPG->SVG
# Template: SVG Graphics Generator & Converter
# Opis: Tworzenie tła certyfikatów/reklam oraz konwersja grafiki JPG na SVG
#===============================================================================

set -euo pipefail

PROJECT_NAME="${1:-svg-graphics-tool}"
PROJECT_DIR="${2:-./${PROJECT_NAME}}"

echo "🎨 Tworzenie projektu SVG Graphics: $PROJECT_NAME"
echo "Lokalizacja: $PROJECT_DIR"

# Tworzenie struktury katalogów
mkdir -p "$PROJECT_DIR"/{src,output,samples,docs}

#===============================================================================
# Główny moduł Python: generator_svg.py
#===============================================================================

cat > "$PROJECT_DIR/src/generator_svg.py" << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
...
chmod +x "$PROJECT_DIR/src/generator_svg.py"

#===============================================================================
# Przykładowe skrypty bash
#===============================================================================

cat > "$PROJECT_DIR/generate_certificate.sh" << 'BASH_EOF'
#!/bin/bash
...
chmod +x "$PROJECT_DIR/generate_certificate.sh"

cat > "$PROJECT_DIR/generate_advertisement.sh" << 'BASH_EOF'
#!/bin/bash
...
chmod +x "$PROJECT_DIR/generate_advertisement.sh"

cat > "$PROJECT_DIR/convert_image.sh" << 'BASH_EOF'
#!/bin/bash
...
chmod +x "$PROJECT_DIR/convert_image.sh"

#===============================================================================
# Przykładowe pliki
#===============================================================================

cat > "$PROJECT_DIR/samples/README.md" << 'EOF'
...
cat > "$PROJECT_DIR/docs/USAGE.md" << 'EOF'
...
pip3 install svgwrite pillow 2>/dev/null || echo "⚠️  Nie udało się zainstalować zależności (może wymagać uprawnień)"

echo ""
echo "🧪 Uruchamianie testu..."
cd "$PROJECT_DIR"

# Generuj przykładowe tło certyfikatu
python3 src/generator_svg.py certificate \
    --output samples/certificate_sample.svg \
    --color "#1a5f7a" \
    --pattern circles

# Generuj przykładowe tło reklamy
python3 src/generator_svg.py advertisement \
    --output samples/advertisement_sample.svg \
    --colors "#667eea,#764ba2"
...
python3 src/generator_svg.py placeholder \
    --width 400 \
    --height 300 \
    --output samples/placeholder_sample.svg \
    --text "Sample"

echo ""
echo "✅ Projekt został utworzony pomyślnie!"
echo ""
echo "📂 Lokalizacja: $PROJECT_DIR"
echo ""
echo "📝 Wygenerowane przykłady:"
ls -la "$PROJECT_DIR/samples/"*.svg 2>/dev/null || echo "   (brak plików SVG)"
echo ""
echo "🚀 Aby użyć:"
echo "   cd $PROJECT_DIR"
echo "   ./generate_certificate.sh moj_certyfikat.svg"
echo "   ./generate_advertisement.sh moja_reklama.svg"
echo "   ./convert_image.sh obrazek.jpg"
echo ""
